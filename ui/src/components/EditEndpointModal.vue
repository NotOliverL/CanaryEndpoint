        ...
        <form method="dialog" class="modal-box" @submit.prevent="submit">
            ...
            <div v-if="responseType === 'IMAGE'" class="form-control mb-2">
                ...
                <div v-if="existingImage" class="mt-2">
                    ...
                    <img
                        :src="existingImage"
                        class="max-h-32 rounded block mt-2"
                    />
                </div>
            </div>
            ...
            <div
                v-if="message"
                :class="isError ? 'text-error mt-3' : 'text-success mt-3'"
            >
                {{ message }}
            </div>
        </form>
...
watch(
    ...
    (ep) => {
        if (ep) {
            ...
            existingSuccessImage.value =
                ep.response_image_referer_success || null;
            existingFailureImage.value =
                ep.response_image_referer_failure || null;
...
const submit = async () => {
    ...
    try {
        ...
        if (file && responseType.value === "IMAGE")
        ...
        isError.value = true;
        message.value =
            err?.response?.data || err?.message || "Error updating endpoint";
    }
};
</script>
