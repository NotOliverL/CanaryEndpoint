  ...
  <dialog :open="open" class="modal">
    <form method="dialog" class="modal-box" @submit.prevent="submit">
        ...
        <div class="form-control mb-2 flex items-center">
            ...
            <input id="name" v-model="name" placeholder="Endpoint name" class="input input-bordered w-full" required />
        </div>

        <div class="form-control mb-2 flex items-center">
            <label class="label w-26 text-sm">Path</label>
            <input id="path" v-model="path" placeholder="URL Path" class="input input-bordered w-full" required />
        </div>

        <div class="form-control mb-4 flex items-center">
            <label class="label w-26 text-sm">Description</label>
            <input id="description" v-model="description" placeholder="Description" class="input input-bordered w-full" />
        </div>
        ...
        <div class="form-control mb-2">
            ...
            <select v-model="responseMethod" class="select select-bordered w-full">
        ...
        <div v-if="message" :class="isError ? 'text-error' : 'text-success'">{{ message }}</div>
...
const submit = async () => {
    ...
    if (responseMethod.value === 'REFERER') {
        ...
        if (!fileSuccess || !fileFailure) {
            isError.value = true
            message.value = 'Both success and failure images are required for Referer response.'
            return
        }
    }
    ...
    try {
        ...
        isError.value = true
        message.value = err?.response?.data || err?.message || 'Error creating endpoint'
    }
}
</script>