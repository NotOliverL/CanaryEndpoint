<template>
    <header class="navbar bg-base-300 px-4 mb-2 shadow-m flex items-center">
        <div class="flex-1 flex items-center">
            <img
                class="w-8 h-8 mr-2 pointer-events-none select-none"
                src="../assets/Picture 1.png"
                alt="logo"
            />
            <a class="text-xl font-bold cursor-default select-none"
                >Canary Endpoint</a
            >
        </div>
        <div class="flex-none">
            <ul class="menu menu-horizontal px-1 gap-1">
                <li>
                    <button
                        ...
                        @click="toggleTheme"
                        :title="`Current: ${
                            currentTheme === 'winter' ? 'Light' : 'Dark'
                        } - Click to switch`"
                    >
                ...
                <li>
                    <button
                        class="bg-red-400 hover:bg-red-500 px-2 py-1 rounded text-sm font-semibold ml-1"
                        @click="logout"
                    >
                        Logout
                    </button>
