...
class Endpoint(models.Model):
    ...
    response_type = models.CharField(
        max_length=20, choices=RESPONSE_CHOICES, default=RESPONSE_NOT_FOUND
    )
    response_redirect_url = models.CharField(max_length=1000, blank=True, null=True)
    response_image = models.ImageField(
        upload_to="endpoint_images/", blank=True, null=True
    )
    response_image_referer_success = models.ImageField(
        upload_to="endpoint_images/", blank=True, null=True
    )
    response_image_referer_failure = models.ImageField(
        upload_to="endpoint_images/", blank=True, null=True
    )
